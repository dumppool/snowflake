...
namespace LDecoder
{
	...
	struct FVideoState
	{
		...
		std::thread LoopThreadId;
		std::thread AudioThreadId;

		AVInputFormat* InputFormat;
		AVFormatContext* Ic;

		IDecodeCallback* DecodeCB;
	...
	private:
		...
		void PushCommand(const std::function<void()>& cmd);
		...
		int32 Execute_ReadThread(void* arg);
		int32 Execute_LoopThread(void* arg);
		//int32 Execute_AudioThread(void* arg);
		void Execute_AudioThread(void* arg);
		void Refresh_LoopThread(FVideoState* is, double* remaining);
