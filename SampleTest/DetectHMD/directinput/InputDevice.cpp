...
namespace lostvr
{
	...
	void InputDevice::PollKeyboardState()
	{
		...
		CHAR strPressed[1024] = { 0 };
		CHAR strReleased[1024] = { 0 };
		for (int i = 0; i < 256; ++i)
		{
			if (*((UCHAR*)KeyStates + i) != *((UCHAR*)KeyStates + i + 256))
			{
				...
				{
					snprintf(strReleased, 1023, "%s, %c(%d), \'a\': (%c)(%d)", strReleased, i, i, 'a', 'a');
				...
				{
					snprintf(strPressed, 1023, "%s, %c(%d), \'1\': (%c)(%d)", strPressed, i, i, '1', '1');
					for (auto func : KeyboardCallback[i])
					{
						if ((PFN_KeyboardCallback)func != nullptr)
						{
		...
		if (strReleased[0] != 0 || strPressed[0] != 0)
		{
			LVMSG2(SLogPrefix, head, "pressed: %s, released: %s", strPressed, strReleased);
		}
	}
