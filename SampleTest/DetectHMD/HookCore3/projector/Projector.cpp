...
{
	//LVMSG("TextureProjector::OnPresent", "SourceRef(%x), Source(%x)", SourceRef, Source);
	LVASSERT(Source != nullptr, "TextureProjector::OnPresent", "the input source is none");
	if (SourceRef == nullptr || SourceRef != Source)
	{
		return false;
	}
...
bool TextureProjector::Init(const ID3D11Resource* Source, uint32 HmdBufWidth, uint32 HmdBufHeight, const LVMatrix* LeftEyeView, const LVMatrix* LeftEyeProj, const LVMatrix* RightEyeView, const LVMatrix* RightEyeProj)
{
	HRESULT Res;

	SourceRef = (ID3D11Texture2D*)Source;
	SourceRef->AddRef();

	BufferWidth = HmdBufWidth;
	BufferHeight = HmdBufHeight;

	if (LeftEyeView != nullptr && LeftEyeProj != nullptr)
	{
		SetEyePose(0, *LeftEyeView, *LeftEyeProj);
	}

	if (RightEyeView != nullptr && RightEyeProj != nullptr)
	{
		SetEyePose(1, *RightEyeView, *RightEyeProj);
	}
	
	ID3D11Device* SrcDev = nullptr;
	SourceRef->GetDevice(&SrcDev);
	DeviceRef = SrcDev;
