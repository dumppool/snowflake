...
// global DX11 state
static struct DirectX11 SDXINST;
...
struct Texture
{
    ...
    void Init(int sizeW, int sizeH, bool rendertarget, int mipLevels, int sampleCount)
    {
        ...
        SDXINST.Device->CreateTexture2D(&dsDesc, NULL, &Tex);
        SDXINST.Device->CreateShaderResourceView(Tex, NULL, &TexSv);
        TexRtv = nullptr;
        if (rendertarget) SDXINST.Device->CreateRenderTargetView(Tex, NULL, &TexRtv);
    }
    ...
    void FillTexture(uint32_t * pix)
    {
        ...
        {
            SDXINST.Context->UpdateSubresource(Tex, level, NULL, (unsigned char *)pix, sizeW * 4, sizeH * sizeW * 4);
...
struct Material
{
        ...
        char* vertexShader = NULL, char* pixelShader = NULL, int vSize = 24) : Tex(t), VertexSize(vSize)
    {
        ...
        SDXINST.Device->CreateVertexShader(blobData->GetBufferPointer(), blobData->GetBufferSize(), NULL, &VertexShader);

        // Create input layout
        SDXINST.Device->CreateInputLayout(vertexDesc, numVertexDesc,
        ...
        SDXINST.Device->CreateVertexShader(blobData->GetBufferPointer(), blobData->GetBufferSize(), NULL, &VertexShaderInstanced);
        blobData->Release();

        // Create pixel shader
        D3DCompile(pixelShader, strlen(pixelShader), 0, 0, 0, "main", "ps_4_0", 0, 0, &blobData, 0);
        SDXINST.Device->CreatePixelShader(blobData->GetBufferPointer(), blobData->GetBufferSize(), NULL, &PixelShader);
        blobData->Release();
        ...
        ss.MaxAnisotropy = 8;
        ss.MaxLOD = 15;
        SDXINST.Device->CreateSamplerState(&ss, &SamplerState);
        ...
        SDXINST.Device->CreateRasterizerState(&rs, &Rasterizer);
        ...
        SDXINST.Device->CreateDepthStencilState(&dss, &DepthState);
        ...
        SDXINST.Device->CreateBlendState(&bm, &BlendState);
    }
    ~Material()
    {
...
struct Model
{
    ...
    void Init(TriangleSet * t)
    {
        ...
        VertexBuffer = new DataBuffer(SDXINST.Device, D3D11_BIND_VERTEX_BUFFER, &t->Vertices[0], t->numVertices * sizeof(Vertex));
        IndexBuffer = new DataBuffer(SDXINST.Device, D3D11_BIND_INDEX_BUFFER, &t->Indices[0], t->numIndices * sizeof(short));
    }
    ...
    void Render(XMMATRIX * projView, float R, float G, float B, float A, bool standardUniforms)
    {
        ...
        float col[] = { R, G, B, A };
        if (standardUniforms) memcpy(SDXINST.UniformData + 0, &mat, 64); // ProjView
        if (standardUniforms) memcpy(SDXINST.UniformData + 64, &col, 16); // MasterCol
        D3D11_MAPPED_SUBRESOURCE map;
        SDXINST.Context->Map(SDXINST.UniformBufferGen->D3DBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
        memcpy(map.pData, &SDXINST.UniformData, SDXINST.UNIFORM_DATA_SIZE);
        SDXINST.Context->Unmap(SDXINST.UniformBufferGen->D3DBuffer, 0);
        SDXINST.Context->IASetInputLayout(Fill->InputLayout);
        SDXINST.Context->IASetIndexBuffer(IndexBuffer->D3DBuffer, DXGI_FORMAT_R16_UINT, 0);
        UINT offset = 0;
        SDXINST.Context->IASetVertexBuffers(0, 1, &VertexBuffer->D3DBuffer, &Fill->VertexSize, &offset);
        SDXINST.Context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        SDXINST.Context->VSSetShader(Fill->VertexShader, NULL, 0);
        SDXINST.Context->PSSetShader(Fill->PixelShader, NULL, 0);
        SDXINST.Context->PSSetSamplers(0, 1, &Fill->SamplerState);
        SDXINST.Context->RSSetState(Fill->Rasterizer);
        SDXINST.Context->OMSetDepthStencilState(Fill->DepthState, 0);
        SDXINST.Context->OMSetBlendState(Fill->BlendState, NULL, 0xffffffff);
        SDXINST.Context->PSSetShaderResources(0, 1, &Fill->Tex->TexSv);

        SDXINST.Context->DrawIndexed((UINT)NumIndices, 0, 0);
    }
    ...
    void RenderInstanced(XMMATRIX *viewProjMats, float R, float G, float B, float A, bool standardUniforms)
    {
        ...
        float col[] = { R, G, B, A };
        if (standardUniforms) {
            memcpy(SDXINST.UniformData + 0, &modelViewProjMatL, 64);
            memcpy(SDXINST.UniformData + 64, &modelViewProjMatR, 64);
            memcpy(SDXINST.UniformData + 128, &col, 16); // MasterCol
        }
        D3D11_MAPPED_SUBRESOURCE map;
        SDXINST.Context->Map(SDXINST.UniformBufferGen->D3DBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
        memcpy(map.pData, &SDXINST.UniformData, SDXINST.UNIFORM_DATA_SIZE);
        ...
        SDXINST.Context->IASetVertexBuffers(0, 1, &VertexBuffer->D3DBuffer, &Fill->VertexSize, &offset);
        SDXINST.Context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        SDXINST.Context->VSSetShader(Fill->VertexShaderInstanced, NULL, 0);
        SDXINST.Context->PSSetShader(Fill->PixelShader, NULL, 0);
        SDXINST.Context->PSSetSamplers(0, 1, &Fill->SamplerState);
        SDXINST.Context->RSSetState(Fill->Rasterizer);
        SDXINST.Context->OMSetDepthStencilState(Fill->DepthState, 0);
        ...
        // draw 2 instances
        SDXINST.Context->DrawIndexedInstanced((UINT)NumIndices, 2, 0, 0, 0);
    }
...
namespace lostvr {
	...
	class D3D11Projector : public IRenderer
	{
	public:
		D3D11Projector();
		~D3D11Projector();
		...
		static D3D11Projector* Get()
		{
			static D3D11Projector Inst;
			return &Inst;
		}
