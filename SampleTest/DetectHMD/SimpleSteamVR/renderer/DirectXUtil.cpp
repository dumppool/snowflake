...
bool D3D11Projector::OnPresent(const ID3D11Resource* Source)
{
	Update();
	return true;
}

bool D3D11Projector::Init(ID3D11Device* InDevice)
{
	HRESULT Res;
...
ID3D11Texture2D* D3D11Projector::GetFinalBuffer(EnumEyeID Eye)
{
	return BB[Eye];
}
...
void D3D11Projector::SetEyePose(EnumEyeID Eye, const LVMatrix& EyeView, const LVMatrix& Proj)
{
...
bool D3D11Projector::SetSourceRef(ID3D11Texture2D* InSource)
{
