...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (vp->bmp) {
...
#else
        {
            ...
            if (e) {
                const AVClass * cls = sws_get_class();
                const AVOption    *o = av_opt_find(&cls, "sws_flags", NULL, 0,
                                                   AV_OPT_SEARCH_FAKE_OBJ);
                int ret = av_opt_eval_flags(&cls, o, e->value, (int*)&sws_flags);
                if (ret < 0)
                    exit(1);
            }
        }
...
static int opt_show_mode(void *optctx, const char *opt, const char *arg)
{
    show_mode = !strcmp(arg, "video") ? VideoState::SHOW_MODE_VIDEO :
                ...
                parse_number_or_die(opt, arg, OPT_INT, 0, VideoState::SHOW_MODE_NB-1);
    return 0;
}
